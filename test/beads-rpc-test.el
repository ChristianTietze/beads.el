;;; beads-rpc-test.el --- Tests for beads-rpc.el -*- lexical-binding: t; -*-
...
;; their test data via beads-rpc-delete to avoid polluting the
...
(ert-deftest beads-rpc-test-find-database-current-dir ()
  "Test that beads-rpc--find-database finds .beads/beads.db in current directory."
  (let ((temp-dir (make-temp-file "beads-test-" t)))
    (unwind-protect
        (let ((beads-dir (expand-file-name ".beads" temp-dir))
          ...
          (should (equal (beads-rpc--find-database)
                        (expand-file-name ".beads/beads.db" temp-dir))))
      (delete-directory temp-dir t))))

(ert-deftest beads-rpc-test-find-database-parent-dir ()
  "Test that beads-rpc--find-database walks up parent directories."
...
(ert-deftest beads-rpc-test-find-database-not-found ()
  "Test that beads-rpc--find-database returns nil when no database found."
  (let ((temp-dir (make-temp-file "beads-test-" t)))
    (unwind-protect
        (let ((default-directory temp-dir))
          (should (null (beads-rpc--find-database))))
      (delete-directory temp-dir t))))

(ert-deftest beads-rpc-test-find-database-beads-db-env ()
  ...
  (let ((temp-dir (make-temp-file "beads-test-" t)))
    (unwind-protect
        (let* ((db-path (expand-file-name "custom/beads.db" temp-dir))
          ...
          (should (equal (beads-rpc--find-database) db-path)))
      (delete-directory temp-dir t))))

(ert-deftest beads-rpc-test-find-database-beads-dir-env ()
...
(ert-deftest beads-rpc-test-find-database-redirect ()
...
(ert-deftest beads-rpc-test-find-database-custom-db-name ()
  ...
  (let ((temp-dir (make-temp-file "beads-test-" t)))
    (unwind-protect
        (let* ((beads-dir (expand-file-name ".beads" temp-dir))
          ...
          (should (equal (beads-rpc--find-database) custom-db)))
      (delete-directory temp-dir t))))

;;; Socket path tests

(ert-deftest beads-rpc-test-socket-path ()
  "Test that beads-rpc--socket-path returns correct path."
  (let ((temp-dir (make-temp-file "beads-test-" t)))
    (unwind-protect
        (let* ((beads-dir (expand-file-name ".beads" temp-dir))
               (db-path (expand-file-name "beads.db" beads-dir))
          ...
          (make-directory beads-dir t)
          (write-region "" nil db-path)
          (should (equal (beads-rpc--socket-path) expected-socket)))
      (delete-directory temp-dir t))))

(ert-deftest beads-rpc-test-socket-path-no-database ()
  "Test that beads-rpc--socket-path returns nil when no database found."
  (let ((temp-dir (make-temp-file "beads-test-" t)))
    (unwind-protect
        (let ((default-directory temp-dir))
          (should (null (beads-rpc--socket-path))))
      (delete-directory temp-dir t))))

;;; Request serialization tests (mock the socket)

(ert-deftest beads-rpc-test-request-structure ()
  "Test that beads-rpc-request creates correct JSON structure."
  (let ((request-data nil))
    (cl-letf (((symbol-function 'beads-rpc--send-to-socket)
               (lambda (_socket-path request)
                 (setq request-data (json-read-from-string request))
                 '((success . t) (data . nil)))))
      (beads-rpc-request "test-operation" '((key . "value")))
...
(ert-deftest beads-rpc-test-request-null-args ()
  "Test that beads-rpc-request handles nil args correctly."
  (let ((request-data nil))
    (cl-letf (((symbol-function 'beads-rpc--send-to-socket)
               (lambda (_socket-path request)
                 (setq request-data (json-read-from-string request))
                 '((success . t) (data . nil)))))
      (beads-rpc-request "test-operation" nil)
      (should (eq (alist-get 'args request-data) :null)))))

(ert-deftest beads-rpc-test-request-filter-encoding ()
  ...
  (let ((request-data nil))
    (cl-letf (((symbol-function 'beads-rpc--send-to-socket)
               (lambda (_socket-path request)
                 (setq request-data (json-read-from-string request))
                 '((success . t) (data . nil)))))
      (beads-rpc-request "list" '((status . "open")
                                  (priority . 1)
...
(ert-deftest beads-rpc-test-request-error-handling ()
  "Test that beads-rpc-request handles error responses."
  (cl-letf (((symbol-function 'beads-rpc--send-to-socket)
             (lambda (_socket-path _request)
               '((success . :json-false) (error . "Test error message")))))
    (should-error (beads-rpc-request "test-operation" nil)
                  :type 'beads-rpc-error)))

;;; Integration tests (require daemon)

(ert-deftest beads-rpc-test-health ()
  "Test that beads-rpc-health returns healthy status from daemon."
  :tags '(:integration)
  (skip-unless (beads-rpc--socket-path))
  (skip-unless (file-exists-p (beads-rpc--socket-path)))
  (let ((result (beads-rpc-health)))
    (should result)))

(ert-deftest beads-rpc-test-list ()
  "Test that beads-rpc-list returns issues array."
  :tags '(:integration)
  (skip-unless (beads-rpc--socket-path))
  (skip-unless (file-exists-p (beads-rpc--socket-path)))
...
(ert-deftest beads-rpc-test-list-with-filters ()
  "Test that beads-rpc-list accepts filter arguments."
  :tags '(:integration)
  (skip-unless (beads-rpc--socket-path))
  (skip-unless (file-exists-p (beads-rpc--socket-path)))
  (let ((issues (beads-rpc-list '(:status "open" :priority 1))))
    (should (vectorp issues))))

(ert-deftest beads-rpc-test-ready ()
  "Test that beads-rpc-ready returns unblocked issues."
  :tags '(:integration)
  (skip-unless (beads-rpc--socket-path))
  (skip-unless (file-exists-p (beads-rpc--socket-path)))
  (let ((issues (beads-rpc-ready)))
    (should (vectorp issues))))

(ert-deftest beads-rpc-test-stats ()
  "Test that beads-rpc-stats returns statistics."
  :tags '(:integration)
  (skip-unless (beads-rpc--socket-path))
  (skip-unless (file-exists-p (beads-rpc--socket-path)))
...
(ert-deftest beads-rpc-test-show ()
  "Test that beads-rpc-show returns issue details."
  :tags '(:integration)
  (skip-unless (beads-rpc--socket-path))
  (skip-unless (file-exists-p (beads-rpc--socket-path)))
...
(ert-deftest beads-rpc-test-create-and-close ()
  ...
  (let* ((title "Test issue from ERT")
    ...
    (unwind-protect
        (progn
          (let ((show-issue (beads-rpc-show issue-id)))
            (should (stringp (alist-get 'id show-issue))))
          (let ((closed-issue (beads-rpc-close issue-id "Test cleanup")))
            (should (equal (alist-get 'status closed-issue) "closed"))))
      (beads-rpc-delete (list issue-id) :force t))))

(ert-deftest beads-rpc-test-update ()
  "Test updating an issue via RPC."
  :tags '(:integration :destructive)
  (skip-unless (beads-rpc--socket-path))
  (skip-unless (file-exists-p (beads-rpc--socket-path)))
  (let* ((issue (beads-rpc-create
                 "Test update issue"
                 :priority 2))
         (issue-id (alist-get 'id issue)))
    (unwind-protect
        (let* ((updated-issue (beads-rpc-update
                               issue-id
                               :status "in_progress"
                               :priority 1
                               :notes "Working on it"))
               (show-issue (beads-rpc-show issue-id)))
          (should (equal (alist-get 'status updated-issue) "in_progress"))
          (should (equal (alist-get 'priority show-issue) 1)))
      (beads-rpc-delete (list issue-id) :force t))))

(ert-deftest beads-rpc-test-count ()
  "Test that beads-rpc-count returns grouped counts."
  :tags '(:integration)
  (skip-unless (beads-rpc--socket-path))
  (skip-unless (file-exists-p (beads-rpc--socket-path)))
  (let ((counts (beads-rpc-count '(:group-by "status"))))
    (should (listp counts))))

(ert-deftest beads-rpc-test-dep-add-remove ()
  "Test adding and removing dependencies via RPC."
  :tags '(:integration :destructive)
  (skip-unless (beads-rpc--socket-path))
  (skip-unless (file-exists-p (beads-rpc--socket-path)))
  (let* ((issue1 (beads-rpc-create "Dependency test 1"))
         (issue2 (beads-rpc-create "Dependency test 2"))
         (id1 (alist-get 'id issue1))
         (id2 (alist-get 'id issue2)))
    (unwind-protect
        (progn
          (let ((_add-result (beads-rpc-dep-add id1 id2 "blocks")))
            (should t))
          (let ((_show-issue (beads-rpc-show id1)))
            (should t))
          (let ((_remove-result (beads-rpc-dep-remove id1 id2)))
            (should t)))
      (beads-rpc-delete (list id1 id2) :force t))))

(ert-deftest beads-rpc-test-label-operations ()
  "Test adding and removing labels via RPC."
  :tags '(:integration :destructive)
  (skip-unless (beads-rpc--socket-path))
  (skip-unless (file-exists-p (beads-rpc--socket-path)))
  (let* ((issue (beads-rpc-create "Label test"))
         (issue-id (alist-get 'id issue)))
    (unwind-protect
        (progn
          (let ((_add-result (beads-rpc-label-add issue-id "test-label")))
            (should t))
          (let* ((show-issue (beads-rpc-show issue-id))
                 (labels (alist-get 'labels show-issue)))
            (should (member "test-label" (append labels nil))))
          (let ((_remove-result (beads-rpc-label-remove issue-id "test-label")))
            (should t)))
      (beads-rpc-delete (list issue-id) :force t))))

(ert-deftest beads-rpc-test-connection-timeout ()
  "Test that connection timeout is handled gracefully."
  :tags '(:integration)
  (let ((_beads-rpc-timeout 0.1))
    (cl-letf (((symbol-function 'beads-rpc--socket-path)
               (lambda () "/tmp/nonexistent-socket.sock")))
      (should-error (beads-rpc-health)
                    :type 'beads-rpc-error))))

(ert-deftest beads-rpc-test-invalid-operation ()
  "Test that invalid operations are handled."
  :tags '(:integration)
  (skip-unless (beads-rpc--socket-path))
  (skip-unless (file-exists-p (beads-rpc--socket-path)))
  (should-error (beads-rpc-request "invalid-operation" nil)
                :type 'beads-rpc-error))

(provide 'beads-rpc-test)
;;; beads-rpc-test.el ends here
